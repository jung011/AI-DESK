...
> AI가 도구 호출로 능동 발신/응답 가능하도록. **M5 달성**.
...
- ✅ Claude Code 실제 등록 후 도구 호출 검증 — 4 도구 호출 + 답장 체인 + 부모 자동 replied + tmux 도착 모두 통과
...
- 2026-05-09 : Phase 5 첫 라운드 — aidesk-channel MCP 서버 (4 도구 + `<channel>` 5초 폴링 push + 등록 가이드) 작성. 기동 smoke 통과.
- 2026-05-09 : Claude Code 새 세션에 mcp.json 등록 후 도구 호출 검증 — list_agents·check_inbox·send_to·reply 모두 동작, 답장 체인 reply_to_message_id 매핑 + 부모 자동 status=replied 갱신 + tmux 도착 모두 확인 — **M5 달성**. 1단계 5 마일스톤 모두 완료.
