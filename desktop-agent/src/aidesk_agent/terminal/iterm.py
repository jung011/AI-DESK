...
def build_open_iterm_script(
    ...
    return (
        ...
        '  end if\n'
        '  if not foundIt then\n'
        '    -- delay 1.0 동안 iTerm frontmost 가 직전 창으로 복귀하는 race 가 있어\n'
        '    -- *current window* 재참조 금지. newWin / newSes 객체 reference 로\n'
        '    -- 명시해서 OSC / set name / write text 가 엉뚱한 me 창으로 폴백되는 사고 차단.\n'
        '    -- (실증: me + 새 AI 외부터미널 연속 열기 시 새 AI 명령이 me 입력 프롬프트로 주입됨)\n'
        '    set newWin to (create window with profile "AI Desk")\n'
        '    set newSes to current session of newWin\n'
        '    tell newSes to write text shellCmd\n'
        ...
        '      tell newSes to write text (ESC & "]0;" & tabTitle & BEL) without newline\n'
        '    end try\n'
        '    try\n'
        '      set name of newSes to tabTitle\n'
        '    end try\n'
        '    try\n'
        '      set name of current tab of newWin to tabTitle\n'
        '    end try\n'
        '  end if\n'
